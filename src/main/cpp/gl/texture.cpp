...
public:
  ...
  void Bind(std::shared_ptr<TextureObject> object) { m_boundObject = object; }
...
void internal_convert(GLenum *internal_format, GLenum *type, GLenum *format) {
  ...
  case GL_DEPTH_COMPONENT:
    LOG_D("Find GL_DEPTH_COMPONENT: internalFormat: %s, format: %s, type: %s",
          glEnumToString(*internal_format), glEnumToString(*format),
          glEnumToString(*type));
    if (type) {
  ...
  case GL_COMPRESSED_RG_RGTC2:
    LOG_E(
        "GL_COMPRESSED_RED_RGTC1 or GL_COMPRESSED_RG_RGTC2 is not supported!");
    break;
  case GL_SRGB8:
    if (type)
...
void glTexImage1D(GLenum target, GLint level, GLint internalFormat,
  ...
  auto tex = GetTextureUnit(currentUnitIndex)
                 .GetBindingSlot(ConvertGLEnumToTextureTarget(target))
                 .GetBoundObject();
...
void glTexImage2D(GLenum target, GLint level, GLint internalFormat,
  ...
  if (transfer_format == GL_BGRA && tex->format != transfer_format &&
      internalFormat == GL_RGBA8 && width <= 128 &&
      height <= 128) { // xaero has 64x64 tiles...hack here
    LOG_D("Detected GL_BGRA format @ tex = %d, do swizzle", tex->texture)
    if (tex->swizzle_param[0] ==
        0) { // assert this as never called glTexParameteri(...,
  ...
  // Fix for 1.12
    ...
    {
        LOG_D("Detected GL_BGRA/GL_UNSIGNED_INT_8_8_8_8_REV, do swizzle")
  ...
  GLES.glTexImage2D(target, level, internalFormat, width, height, border,
                    format, type, pixels);
...
void glTexImage3D(GLenum target, GLint level, GLint internalFormat,
  ...
  GLES.glTexImage3D(target, level, internalFormat, width, height, depth, border,
                    format, type, pixels);
...
void glCopyTexImage2D(GLenum target, GLint level, GLenum internalFormat,
  ...
  GLES.glGetTexLevelParameteriv(target, level, GL_TEXTURE_INTERNAL_FORMAT,
                                &realInternalFormat);
  ...
  if (is_depth_format(internalFormat)) {
    ...
    GLES.glTexImage2D(target, level, (GLint)internalFormat, width, height,
    ...
    CHECK_GL_ERROR_NO_INIT
    glFramebufferTexture2D(GL_DRAW_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, target,
                           currentTex, level);
    CHECK_GL_ERROR_NO_INIT
    ...
    GLES.glBlitFramebuffer(x, y, x + width, y + height, 0, 0, width, height,
    ...
    GLES.glCopyTexImage2D(target, level, internalFormat, x, y, width, height,
                          border);
    CHECK_GL_ERROR_NO_INIT
  }
...
void glCopyTexSubImage2D(GLenum target, GLint level, GLint xoffset,
  ...
  if (is_depth_format((GLenum)internalFormat)) {
    ...
    GLES.glBlitFramebuffer(x, y, x + width, y + height, xoffset, yoffset,
    ...
    GLES.glCopyTexSubImage2D(target, level, xoffset, yoffset, x, y, width,
                             height);
  }
...
void glRenderbufferStorage(GLenum target, GLenum internalFormat, GLsizei width,
  ...
  GLint realInternalFormat;
  GLES.glGetTexLevelParameteriv(target, 0, GL_TEXTURE_INTERNAL_FORMAT,
                                &realInternalFormat);
  ERR = GLES.glGetError();
...
void glRenderbufferStorageMultisample(GLenum target, GLsizei samples,
  ...
  GLES.glRenderbufferStorageMultisample(target, samples, internalFormat, width,
                                        height);
...
void glGetTexLevelParameterfv(GLenum target, GLint level, GLenum pname,
                              GLfloat *params) {
  LOG()
  LOG_D("glGetTexLevelParameterfv,target: %d, level: %d, pname: %d", target,
        level, pname)
  GLenum rtarget = map_tex_target(target);
  if (rtarget == GL_PROXY_TEXTURE_2D) {
    switch (pname) {
...
void glGetTexLevelParameteriv(GLenum target, GLint level, GLenum pname,
                              GLint *params) {
  LOG()
  LOG_D("glGetTexLevelParameteriv,target: %s, level: %d, pname: %s",
        glEnumToString(target), level, glEnumToString(pname))
  GLenum rtarget = map_tex_target(target);
  if (rtarget == GL_PROXY_TEXTURE_2D) {
    switch (pname) {
    case GL_TEXTURE_WIDTH:
  ...
  LOG_D("es.glGetTexLevelParameteriv,target: %s, level: %d, pname: %s",
        glEnumToString(target), level, glEnumToString(pname))
...
void glTexParameteriv(GLenum target, GLenum pname, const GLint *params) {
  LOG_D("glTexParameteriv, target: %s, pname: %s, params[0]: %s", params,
        glEnumToString(pname), params ? glEnumToString(params[0]) : "0")
...
void glBindTexture(GLenum target, GLuint texture) {
  ...
  auto &bindingSlot =
      currentUnit.GetBindingSlot(ConvertGLEnumToTextureTarget(target));
...
void glGetTexImage(GLenum target, GLint level, GLenum format, GLenum type,
  ...
  if (fboStatus != GL_FRAMEBUFFER_COMPLETE) {
    LOG_E("glGetTexImage: Failed to create complete framebuffer. Status: 0x%x",
          fboStatus)
    glBindFramebuffer(GL_FRAMEBUFFER, prevFBO);
    glDeleteFramebuffers(1, &tempFBO);
    return;
...
void glReadPixels(GLint x, GLint y, GLsizei width, GLsizei height,
  ...
  if (prevFormat == GL_BGRA && type == GL_UNSIGNED_BYTE) {
    ...
    std::fstream fs(
        std::string(concatenate(mg_directory_path, "/readpixels/")) +
...
void glTexParameteri(GLenum target, GLenum pname, GLint param) {
  ...
  if (pname == GL_TEXTURE_LOD_BIAS_QCOM &&
      !g_gles_caps.GL_QCOM_texture_lod_bias) {
    LOG_D("Does not support GL_QCOM_texture_lod_bias, skipped!")
    return;
  }
...
void glClearTexImage(GLuint texture, GLint level, GLenum format, GLenum type,
  ...
  if (data != nullptr) {
    if (format == GL_RGBA && type == GL_UNSIGNED_BYTE) {
      ...
      GLES.glClearColor(
          (float)byteData[0] / 255.0f, (float)byteData[1] / 255.0f,
          (float)byteData[2] / 255.0f, (float)byteData[3] / 255.0f);
    } else if (format == GL_RGB && type == GL_UNSIGNED_BYTE) {
      auto *byteData = static_cast<const GLubyte *>(data);
      GLES.glClearColor((float)byteData[0] / 255.0f,
                        (float)byteData[1] / 255.0f,
                        (float)byteData[2] / 255.0f, 1.0f);
